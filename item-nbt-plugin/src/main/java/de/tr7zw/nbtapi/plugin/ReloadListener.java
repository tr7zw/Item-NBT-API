...
/**
 * This listener class tries to prevent people from reloading while the
 * NBTInjector is enabled.
 * 
 * @author tr7zw
 *
 */
    ...
    public void onCommand(org.bukkit.event.server.ServerCommandEvent event) {
        if (event.getCommand().toLowerCase().startsWith("reload") && NBTInjector.isInjected()) {
            ...
            event.getSender().sendMessage(
                    "[NBTAPI] The NBTInjector is currently enabled. Reloading will turn the server into an unstable state and data-loss may accure. Please do a clean restart. Canceled reload!");
        }
    }
