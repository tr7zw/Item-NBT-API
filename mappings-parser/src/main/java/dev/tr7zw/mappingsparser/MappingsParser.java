...
/**
 * Internal class to be used to port SpecialSource mapping txt files to nbt-api internal mappings
 * 
 * @author tr7zw
 *
 */
public class MappingsParser {
    ...
    public static void main(String[] args) throws IOException {
        File input = new File("minecraft_server.1.18-pre8.txt");
        ...
        for(String line : lines) {
            ...
            for(ReflectionMethod method : mappedNames.keySet()) {
                if(method.getSelectedVersionInfo() == null)continue;
                if(!mojangName.contains("("))continue;
    ...
    public static void proccessMapping(ClassWrapper nmsWrapper, Map<ReflectionMethod, String> mappedNames) {
        for(Entry<ReflectionMethod, String> entry : mappedNames.entrySet()) {
            if(entry.getValue() == null) {
                System.out.println("Missing mapping in class " + nmsWrapper.getMojangName() + " method " + entry.getKey().name());
            } else {
                builder.append("put(\"" + nmsWrapper.getMojangName() + "#" + entry.getKey().getSelectedVersionInfo().name + "\", \"" + entry.getValue() + "\");\n");
            }
        }
    }
